...
One century ago, lexical availability studies raised as a way to identify relevant vocabulary. Necessity of formal background that enforce the elections was soon found relevant. Since then, quantitative models for expressing the relevancy of the words into a interest center has been developed.

The last of a serie of modifications is the proposition of López-Strassbuger formula, which offers a nice quantitative approximation to evaluation of lexical centrality and relevance of words in a so-called "interest center".

In Ávila-Sánchez a new approach of this subject is proposed. Actually, it's just a revision of the concepts treated in previous works and focusing in developing a model based approach to the process of evaluation. In this order, a new interpretation of the availability tests is called. More than a availability studies, tests are a prototyping identification tests. A prototype is offered to the speaker as a entry point to their lexical network. This is not a criticism to the probe or validity of studies, but a way to get a deeper understanding about what's happening. 

Next, some concepts are revised and options offered that could be used to evaluate the availability tests and will be compared with the previously proposed ones, in concept and performance.

# What's the objetive of a availability study

May be that this is the more important concept that we have to consider, because "what's" determine the subject of the work and the target of study.

In availability tests a starting concept is offered to a speaker, in order to find out which is the vocabulary that is close related to that concept. At this way, the vocabulary from a central concept relevant to a social group is identified. This information could be used to optimize learning process, automatic systems, etc.

Tests are traditionally built around enumerations of terms around the entry concept. It's presumed that  more relevant terms are enunciated sooner than less relevant ones. This is a key concept. Position of terms in lists obtained from each speaker for each interest center is relevant and provide a descendant relevance for each term.

These are traditional cheap and easy tests, but in more sharper studies more information could be added to the system. For example, time when terms are realized. With computer systems it could be possible to obtain that information. Could be informative to realize that a speaker stop enunciating terms and start again after a gap. In previous tests we have observed that speakers went farther and farther from the "center" of the proposed entry point, then returned near to the center and started again the same process.

So, we consider that it's important to split evaluation process in two main parts:

* At first, evaluate each list (with the available information) to provide a evaluation of the relevance of each term in that list.
* The, integrate that information of each speaker in each interest center in order to obtain a global representation of the 
