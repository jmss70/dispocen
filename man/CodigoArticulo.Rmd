...
bs %>%
  head() %>%
  select(SUJETO,GÉNERO,EDAD,AÑOSDEESTUDIO) %>%
  flextable() %>%
  autofit()
```
...
```{r}
disponibilidadHombres <- build.avilasanchez.availability(data %>%
  filter(users %in% idHombres))
...
```{r}
disponibilidadMujeres <- build.avilasanchez.availability(data %>%
  filter(users %in% idMujeres))
